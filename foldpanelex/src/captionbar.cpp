...
void wxCaptionBar::OnMouseEvent(wxMouseEvent& event)
{
    ...
    else if ( ((event.Entering()) && (m_foldIcons)) || (event.Moving()))
    {
        wxPoint pt = event.GetPosition();
        wxRect rect = GetRect();
        int xx, yy;

        xx = rect.GetWidth() - m_iconWidth - m_rightIndent;
        yy = m_iconHeight + m_rightIndent;

        if (  ((m_vertical) && (pt.x > xx ))
            ||((!m_vertical) && (pt.y > yy))
           )
        {
            SetCursor(wxCURSOR_HAND);
    ...
    if(send_event)
    {
        ...
        Update();
        */
        //Send the expand / collapse event to the parent
        wxCommandEvent event(wxEVT_CAPTIONBAR, GetId());
        event.SetEventObject(this);
        if (!m_collapsed) event.SetInt(0); else event.SetInt(1);
